...
def model_lora_keys_unet(model, key_map={}):
    ...
    diffusers_keys = comcells.utils.unet_to_diffusers(model.model_config.unet_config)
    ...
    if isinstance(model, comcells.model_base.SD3): #Diffusers lora SD3
        diffusers_keys = comcells.utils.mmdit_to_diffusers(model.model_config.unet_config, output_prefix="diffusion_model.")
        for k in diffusers_keys:
            if k.endswith(".weight"):
                to = diffusers_keys[k]
    ...
    if isinstance(model, comcells.model_base.AuraFlow): #Diffusers lora AuraFlow
        diffusers_keys = comcells.utils.auraflow_to_diffusers(model.model_config.unet_config, output_prefix="diffusion_model.")
        for k in diffusers_keys:
            if k.endswith(".weight"):
                to = diffusers_keys[k]
    ...
    if isinstance(model, comcells.model_base.HunyuanDiT):
        for k in sdk:
            if k.startswith("diffusion_model.") and k.endswith(".weight"):
                key_lora = k[len("diffusion_model."):-len(".weight")]
                key_map["base_model.model.{}".format(key_lora)] = k #official hunyuan lora format

    if isinstance(model, comcells.model_base.Flux): #Diffusers lora Flux
        diffusers_keys = comcells.utils.flux_to_diffusers(model.model_config.unet_config, output_prefix="diffusion_model.")
        for k in diffusers_keys:
            if k.endswith(".weight"):
                to = diffusers_keys[k]
