...
from comcells.samplers import KSampler
...
#
comcells_graph_list = [DEFAULT_COMFY_GRAPH]
comcells_graph_ids = [DEFAULT_COMFY_GRAPH_ID]
prompt_list = [
    'a painting of a cat',
]
...
class TestInference:
    ...
    def start_client(self, listen:str, port:int):
        # Start client
        comcells_client = ComfyClient()
        # Connect to server (with retries)
        n_tries = 5
        for i in range(n_tries):
            time.sleep(4)
            try:
                comcells_client.connect(listen=listen, port=port)
            except ConnectionRefusedError as e:
                print(e)
                print(f"({i+1}/{n_tries}) Retrying...")
            else:
                break
        return comcells_client
    ...
    @fixture(scope="class", params=comcells_graph_list, ids=comcells_graph_ids, autouse=True)
    def _client_graph(self, request, args_pytest, _server) -> (ComfyClient, ComfyGraph):
        comcells_graph = request.param
        
        # Start client
        comcells_client = self.start_client(args_pytest["listen"], args_pytest["port"])

        # Warm up pipeline
        comcells_client.get_images(graph=comcells_graph.graph, save=False)

        yield comcells_client, comcells_graph
        del comcells_client
        del comcells_graph
        torch.cuda.empty_cache()
    ...
    @fixture
    def comcells_graph(self, _client_graph):
    ...
    def test_comcells(
        ...
        comcells_graph.set_filename_prefix(test_info)
        # Settings for comcells graph
        comcells_graph.set_sampler_name(sampler)
        comcells_graph.set_scheduler(scheduler)
        comcells_graph.set_prompt(prompt)

        # Generate
        images = client.get_images(comcells_graph.graph)
